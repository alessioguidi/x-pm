...
export default async function TermsPage({ params }: { params: Promise<{ org_slug: string }> }) {
  const { org_slug } = await params;
  const { data: org } = await supabase.from('organizations').select('name, page_terms, theme_color').eq('slug', org_slug).single();
  if (!org) notFound();
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16 animate-in fade-in duration-500">
      ...
      <h1 className="text-3xl font-extrabold text-gray-900 mb-8">Termini e Condizioni</h1>
      <div className="prose prose-gray max-w-none text-gray-700 whitespace-pre-line">
        {org.page_terms || "Nessuna informazione disponibile."}
      </div>
    </div>
  );
}
